...
uses
   comctrls, SysUtils, Forms, Windows, Graphics, Classes, Dialogs, Menus, StdCtrls, Controls,
   {ToolEdit,}
   Buttons,
   LUObjects, LUHeader, LUSupport, LUScanDir;
...
const
   ...
   SProcessStop = 'Ïðîöåññ îáðàáîòêè îñòàíîâëåí.';
...
procedure CreateListFileLog(const LogDir: string; PopupMenuLogs: TPopupMenu; ActionViewLogsExecute: TPopupMenuAction);
procedure CreateListFileName(const Path: string;
   PopupMenuLogs: TPopupMenu;
   ActionViewLogsExecute: TPopupMenuAction);
{}
procedure CreateListArchiveYear(PopupMenu: TPopupMenu; ActionExecute: TPopupMenuAction);
...
begin
   if ParamCount > 0 then begin
      ...
      if not FileExists(s) then s := ExpandFileName(IniFileName);
      end
   else begin
...
begin
   ...
   if (StatApplication = saBreak) then raise Exception.Create(SProcessStop);
end;
...
begin
   ...
   for i:=0 to TS.Count-1 do begin
      ...
      if i < TS.Count-1 then Result := Result + '; ';
   end;
end;

procedure CreateListFileLog(const LogDir: string;
   PopupMenuLogs: TPopupMenu;
   ActionViewLogsExecute: TPopupMenuAction);
(*
var
...
(*
   ...
   APPFiles.Free;
*)
   CreateListFileName(IncludeTrailingBackslash(LogDir)+'*.log', PopupMenuLogs, ActionViewLogsExecute);
end;
...
begin
   ...
   *)
   if Control is TSpeedButton then begin
      with (Control as TSpeedButton) do begin
   ...
   if Control is TMenuItem then begin
      with (Control as TMenuItem) do begin
         Enabled := not AReadOnly;
         if AReadOnly then begin
         end;
      end;
   ...
   {
   if Control is TDirectoryEdit then begin
      with (Control as TDirectoryEdit) do begin
         Enabled := not AReadOnly;
         if AReadOnly then begin
            //Color := clBtnFace;
            //Font.Color := clWindowText;
