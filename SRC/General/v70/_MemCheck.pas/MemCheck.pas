...
Version 2.51   -> Also update OutputFileHeader when changing the version #
...
Copyrights...
The authors grant you the right to modify/change the source code as long
as the original authors are mentionned.
Please let us know if you make any improvements, so that we can keep an up
to date version. We also welcome
all comments, preferably by email.

Portions of this file (all the code dealing with TD32 debug information) where
derived from the following work, with permission.
Reuse of this code in a commercial application is not permitted.
The portions are identified by a copyright notice.
...
var
   ...
   UnitsCount: integer;
   OutputFileHeader: string = 'MemCheck version 2.51'#13#10;
...
begin
   ...
   while (Node <> Nil) and (Node.fValue <> Address) do begin
      Owner:= Node;
   ...
   if Node.fBigger = Nil then begin
      ...
      if Node.fSmaller = Nil then begin
         if NodeIsOwnersBigger then
         ...
         MiddleOwner:= Node;
         while Middle.fBigger <> Nil do begin
            MiddleOwner:= Middle;
            Middle:= Middle.fBigger;
         end;
         if Middle = Node.fSmaller then begin
            if NodeIsOwnersBigger then
...
begin
   ...
   if ShowCallStack then begin
      for i:= 1 to StoredCallStackDepth do begin
...
begin
   ...
   if count > MaxDump then Count:= MaxDump;
   Byte(s[0]):= count;
...
begin
   ...
   if ExcludeFirstLevel then begin
      _ESP:= _EBP;
      _EBP:= PInteger(_EBP)^;
   end;
   FillChar(St, SizeOf(St), 0);
   if (_EBP<_ESP) or (_EBP-_ESP>30000) then Exit;
   for i:= 0 to StoredCallStackDepth do begin
      _ESP:= _EBP;
      _EBP:= PInteger(_EBP)^;
      if (_EBP<_ESP) or (_EBP-_ESP>30000) then Exit;
...
begin
   if NbClasses >= Length(AllocatedObjectsClasses) then begin
      UnMemChk;
...
begin
   i:= 0;
   while (i < AllocStatsCount) and (AllocatedObjectsClasses[i] <> TheClass) do Inc(i);
   if i = AllocStatsCount then begin
      if AllocStatsCount = StatsArraysAllocatedPos then begin
         if StatsArraysAllocatedPos = 0 then StatsArraysAllocatedPos:= 10;
...
begin
   if (EBP8_10 = PChar(@TObject.NewInstance)) {comming from TObject.NewInstance ?} then begin
      Result:= OldMemoryManager.GetMem(Size + (SizeOf(TMemoryBlocHeader)));
      if Result = Nil then
         Raise OutOfMemory;
      PMemoryBlocHeader(Result).KindOfBlock:= MClass;
      if EBP36_12 = PChar(@TObject.Create) then begin
      ...
      if EBP32_12 = PChar(@TObject.Create) then begin
         if StoredCallStackDepth > 0 then
            FillCallStack(PMemoryBlocHeader(Result).CallerAddress, True);
      ...
      if EBP_8_13 = AddressOf_NewAnsiString then begin
      ...
      else begin   {Neither an object nor a string, this is a MUser}
         ...
         if StoredCallStackDepth > 0 then
            FillCallStack(PMemoryBlocHeader(Result).CallerAddress, True);
         PMemoryBlocHeader(Result).CallerAddress[0]:= EBP12Caller;
...
var
   Block: PMemoryBlocHeader;
begin
   if PCardinal(PChar(P)-4)^ = Magic then begin
      Block:= PMemoryBlocHeader(PChar(P) - SizeOf(TMemoryBlocHeader));
...
begin
   UnMemChk;
   if NotDestroyedFieldsCount = NotDestroyedFieldsAllocatedSpace then begin
...
begin
   if PCardinal(PChar(P)-4)^ = Magic then begin
      ...
      if IdentifyObjectFields then begin
         if (Block.KindOfBlock = MClass) and
            ...
            for i:= 1 to (Block.AllocatedSize div 4) - 1 do
               If (PInteger(PChar(P) + i * 4)^ > SizeOf(TMemoryBlocHeader)) and
                  CurrentlyAllocatedBlocksTree.Has(PInteger(PChar(P) + i * 4)^ - SizeOf(TMemoryBlocHeader)) then
                  AddFieldInfo(Pointer(PInteger(PChar(P) + i * 4)^ - SizeOf(TMemoryBlocHeader)), TObject(P).ClassType, i);
         UnMemChk;
         if Block.DestructionAdress = Nil then begin
      ...
      if (Block.AllocatedSize > DeallocateFreedMemoryWhenBlockBiggerThan) or
         ...
         (TObject(P) is InstancesConformingToForDeallocation)) then begin
         {we really deallocate the block}
            ...
            if Block.DestructionAdress <> Nil then begin
               ...
               if (Block.KindOfBlock<>MClass) and MemoryBlockCorrupted(P) then begin
                  try
      ...
      if BlocksToShow[Block.KindOfBlock] then begin
         if Block.DestructionAdress = Nil then begin
            ...
            case Block.KindOfBlock of
               MClass:         S:= TObject(PChar(Block)+SizeOf(TMemoryBlocHeader)).ClassName;
               MUser:          S:= 'User';
               MReallocedUser: S:= 'Realloc';
            end;
            if (BlocksToShow[Block.KindOfBlock]) and ((Block.KindOfBlock <> MClass) or
               (TObject(PChar(Block) + SizeOf(TMemoryBlocHeader)) is InstancesConformingToForReporting)) then
               try
                  raise EMemoryLeak.Create(S + ' allocated at '+
                        TextualDebugInfoForAddress(Cardinal(Block.CallerAddress[0]))) at Block.CallerAddress[0];
               except
                  on EMemoryLeak do;
               end;
               if ShowCallStack then
            ...
            {this is not a leak}
            if CheckWipedBlocksOnTermination and (Block.AllocatedSize > 5) and (Block.AllocatedSize <= DoNotCheckWipedBlocksBiggerThan) and
               (not IsMemFilledWithChar(pchar(Block) + SizeOf(TMemoryBlocHeader) + 4,
                    Block.AllocatedSize - 5, CharToUseToWipeOut)) then begin
               try
...
begin
   ...
   for i:= InitContext.InitCount - 1 downto 0 do
      if (Cardinal(@Table^[i].Finit) > Cardinal(@dummy)) and
         (Cardinal(@Table^[i].Finit) - Cardinal(@dummy) < DistToMemCheckFinalization) then begin
   ...
   for i:= SysUtilsFinalizationIndex - 1 downto NewIndexOfSysUtilsFinalization do begin
   ...
   for i:= MemCheckFinalizationIndex - 1 downto NewIndexOfMemcheckFinalization do begin
      ReadProcessMemory(h, pointer(pchar(Table) + i * sizeof(PackageUnitEntry)), CurrentEntry, 8, BytesRead);
      WriteProcessMemory(h, pointer(pchar(Table) + (i + 1) * sizeof(PackageUnitEntry)), CurrentEntry, 8, BytesRead);
   end;
...
var
   LogFileName: string;
begin
   if ExceptProc <> @MyExceptProc then begin
      {not installed yet ?}
      try
...
begin
   if Addresses[0].Address > Address then
      ...
      while Start < Finish - 1 do begin
         Pivot:= Start + (Finish - Start) div 2;
         if Addresses[Pivot].Address = Address then begin
            Start:= Pivot;
            Finish:= Start
            end
         else
...
begin
   if size > 0 then begin
      ...
      for i:=0 to pred(srcmodhdr^._cFile) do begin
         ...
         if srcfile^._nName > 0 then begin
...
begin
   ...
   if (AHeader.Signature <> 'FB09') and (AHeader.Signature <> 'FB0A') then exit;
...
var
   i: integer;
begin
   if not MemCheckInitialized then begin
      {once mechanism}
      SetDispl;
...
begin
   Result:= True;
   i:= 0;
   while (Result) and (i <= StoredCallStackDepth) do begin
...
begin
   ...
   if fWasFieldOfAnotherObject then begin
...
begin
   ...
   Block:= LastBlock;
   while (Block <> Nil) and (LeaksList.Count < MaxNumberOfLeaks) do begin
      if Block.DestructionAdress = Nil then begin
         {this is a leak}
         CurrentLeak:= TLeak.Create(Block);
         if IdentifyObjectFields then
            for i:= 0 to NotDestroyedFieldsCount - 1 do
               if pointer(NotDestroyedFields[i]) = Block then begin
            ...
            NewLeak:= True;
            i:= 0;
            while i < LeaksList.Count do begin
               if LeaksList.Item(i).IsEqual(CurrentLeak) then begin
                  CurrentLeak.Destroy;
...
begin
   ...
   Block:= LastBlock;
   while (Block <> Nil) and (B.Count < MaxNumberOfBlocks) do begin
      if (Block.DestructionAdress <> Nil) and (Block.AllocatedSize > 5) and
         (Block.AllocatedSize <= MaxBlockSize) and
         (not IsMemFilledWithChar(pchar(Block) + SizeOf(TMemoryBlocHeader) + 4,
         Block.AllocatedSize - 5, CharToUseToWipeOut)) then
         B.Add(Block);
      Block:= Block.PreceedingBlock;
   end;
...
begin
   ...
   for i:= ChronogicalInfo.Count - 1 downto 0 do begin
      if (TimeStampsCount > 0) and (ChronogicalInfo.Item(i).Block.LastTimeStamp > LastDisplayedTimeStamp) then begin
   ...
   if CollectStatsAboutObjectAllocation then begin
   ...
   if CheckWipedBlocksOnTermination then begin
      ...
      for i:= 0 to BadBlocks.Count - 1 do begin
...
begin
   InitializeOnce;
   if TimeStampsCount = TimeStampsAllocated then begin
...
begin
   ...
   if not MemCheckActive then begin
      InitializeOnce;
...
begin
   ...
   while Block <> Nil do begin
         BlockToFree:= Block;
         Block:= Block.PreceedingBlock;
         if BlockToFree.DestructionAdress <> Nil then begin
...
begin
   InitializeOnce;
   if UseDebugInfos and (TheAddress > Displ) then begin
