...
type
   TImageComboBox = class;
   TNotifyFileEvent  = procedure (Sender: TImageComboBox; ItemIndex: Integer; var ImageIndex: Integer) of object;
   { TImageComboBox }
   TImageComboBox = class(TCustomComboBox)
   private
      FOnItem: TNotifyFileEvent;
      FImages: TImageList;
   ...
   published
      ...
      property OnItem: TNotifyFileEvent read FOnItem write FOnItem;
   end;
...
begin
   if Value <> FImages then FImages := Value;
   if Assigned(Value) then begin
      ...
      if FAutoCaptions then Items.Clear;
end;
...
begin
   if Assigned(OnDrawItem) then
      ...
      if Assigned(FImages) then begin
         LImageIndex := -1;
         if Assigned(FOnItem) then OnItem(Self, Index, LImageIndex);
         FBitmap := TBitmap.Create;
         FImages.GetBitmap(LImageIndex, FBitmap);
         Canvas.BrushCopy(
...
begin
   ...
   if (ItemIndex > -1) and (ItemIndex < FImages.Count) then begin
      LImageIndex := -1;
      if Assigned(FOnItem) then OnItem(Self, ItemIndex, LImageIndex);
      Result := TBitmap.Create;
      FImages.GetBitmap(LImageIndex, Result);
   end;
end;
