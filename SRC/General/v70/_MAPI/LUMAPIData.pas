...
uses
  DB, Classes, StrUtils, LUSupport, Outlook8, SysUtils,
  LUMAPITags, LUMAPIDefs, LUMAPIProc, MAPI, ComObj;
...
{ TMAPIDataSet }
  ...
  published
    ...
    property FieldDescs: TStringList read FFieldDescs write FFieldDescs;
    property Active;
    property Session: TCDOMAPI read FSession write FSession;
  end;
...
constructor TMAPIDataSet.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FFieldDescs := TStringList.Create;
end;

destructor TMAPIDataSet.Destroy;
begin
  FFieldDescs.Free;
  inherited Destroy;
end;

procedure TMAPIDataSet.InternalOpen;
var
  I: Integer;
begin
...
procedure TMAPIDataSet.InternalClose;
begin
...
function TMAPIDataSet.IsCursorOpen: Boolean;
begin
...
procedure TMAPIDataSet.InternalInitFieldDefs;
begin
//  FieldDefs.Clear;
...
procedure TMAPIDataSet.InternalHandleException;
begin
...
procedure TMAPIDataSet.InternalGotoBookmark(Bookmark: Pointer);
var
  Index: Integer;
begin
...
procedure TMAPIDataSet.InternalSetToRecord(Buffer: PChar);
begin
...
function TMAPIDataSet.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PRecInfo(Buffer + FRecInfoOfs).BookmarkFlag;
end;

procedure TMAPIDataSet.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
...
procedure TMAPIDataSet.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  PInteger(Data)^ := PRecInfo(Buffer + FRecInfoOfs).Bookmark;
end;

procedure TMAPIDataSet.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
...
function TMAPIDataSet.GetRecordSize: Word;
begin
  Result := MaxStrLen;
end;
...
function TMAPIDataSet.AllocRecordBuffer: PChar;
begin
...
procedure TMAPIDataSet.FreeRecordBuffer(var Buffer: PChar);
begin
...
function TMAPIDataSet.GetRecord(Buffer: PChar; GetMode: TGetMode;
...
procedure TMAPIDataSet.InternalInitRecord(Buffer: PChar);
begin
...
function TMAPIDataSet.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
//var
//   i: Integer;
//   Offset: Integer;
begin
(*
   if FileType = ftFixed then begin
      Offset := 0;
      for i:=1 to Field.FieldNo-1 do begin
         Offset := Offset + Fields[i-1].Size;
      end;
      StrLCopy(Buffer, ActiveBuffer+Offset, Field.Size);
      end
   else begin
      StrPCopy(Buffer, Copy(ExtractWordNew(Field.FieldNo, ActiveBuffer, Separator), 1, Field.Size));
   end;
*)
   Result := PChar(Buffer)^ <> #0;
end;

procedure TMAPIDataSet.SetFieldData(Field: TField; Buffer: Pointer);
begin
(*
   if FileType = ftFixed then begin
      Offset := 0;
      ...
      StrLCopy(ActiveBuffer, PChar(Buffer)+Offset, Field.Size);
      end
   else begin
      S := ExtractWordNew(Field.FieldNo, ActiveBuffer, Separator);
   end;
...
procedure TMAPIDataSet.InternalFirst;
begin
  FCurRec := -1;
end;
...
procedure TMAPIDataSet.InternalLast;
begin
  FCurRec := FData.Count;
end;
...
procedure TMAPIDataSet.InternalPost;
begin
  FSaveChanges := True;
...
procedure TMAPIDataSet.InternalAddRecord(Buffer: Pointer; Append: Boolean);
begin
  FSaveChanges := True;
  Inc(FLastBookmark);
...
procedure TMAPIDataSet.InternalDelete;
begin
  FSaveChanges := True;
...
function TMAPIDataSet.GetRecordCount: Longint;
begin
  Result := FData.Count;
end;

function TMAPIDataSet.GetRecNo: Longint;
begin
  UpdateCursorPos;
...
procedure TMAPIDataSet.SetRecNo(Value: Integer);
begin
...
procedure Register;
begin
  RegisterComponents('Luis', [TMAPIDataSet]);
end;
